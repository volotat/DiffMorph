...
for name_id, spec in specs.items():
    ...
    src_resized = cv2.cvtColor(cv2.resize(cv2.cvtColor(src_org, cv2.COLOR_RGB2BGR), (src_org.shape[0], src_org.shape[0])), cv2.COLOR_BGR2RGB)
    trg_resized = cv2.cvtColor(cv2.resize(cv2.cvtColor(trg_org, cv2.COLOR_RGB2BGR), (trg_org.shape[0], trg_org.shape[0])), cv2.COLOR_BGR2RGB)
    ...
    for j, dim in enumerate(needed_dimensions):
        ...
        im = single_image_morpher_resize(morph_class_trained, dim, source_dim, target_dim, scale, name=name, save_images=True)
